...
public class Experiments extends ModuleBase {
	...
	class TranscoderVideoDecoderNotifyExample extends TranscoderVideoDecoderNotifyBase {
		...
		public TranscoderVideoDecoderNotifyExample(int srcWidth, int srcHeight) {
			...
			wowzaImage = new OverlayImage(basePath + graphicName, 100,null, new HashMap<String, String>());
			secondImage = new OverlayImage(basePath + secondGraphName, 100,null, new HashMap<String, String>());
		...
		public void onBeforeScaleFrame(TranscoderSessionVideo sessionVideo, TranscoderStreamSourceVideo sourceVideo,
			...
			synchronized (lock) {
				...
				if (mainImage != null) {
					...
					// does not need to be done for a static graphic, but left here to build on
					...
					if (encodeSource) {
						...
						for (EncoderInfo encoderInfo : encoderInfoList) {
							if (!encoderInfo.destinationVideo.isPassThrough()) {
								...
								encoderInfo.videoPadding[3] = 0;// (int)(((double)videoBottomPadding.getStepValue())*scalingFactor);
																// // bottom
	...
	/*
	 * // calculatedWidth = Math.round(getOverlayPositionX(srcWidth,
	 * thirdPosition)); // int calculatedHeight =
	 * Math.round(getOverlayPositionY(srcHeight, thirdPosition)); this.srcWidth =
	 * srcWidth; wowzaImage = new OverlayImage(basePath+graphicName,100); // if
	 * (calculatedWidth == 0) // calculatedWidth = wowzaImage.GetWidth(1.0); // else
	 * if (calculatedWidth != srcWidth) { // calculatedWidth +=
	 * wowzaImage.GetWidth(0.5); // } // if (calculatedHeight != srcHeight &&
	 * calculatedHeight != 0) { // calculatedHeight += wowzaImage.GetHeight(0.5); //
	 * }
	 * 
	 * //Create the Wowza logo image
	 * getLogger().info("screen width="+srcWidth+" height = "+srcHeight);
	 * secondImage = new OverlayImage(basePath+lBandImagePath,100); holderImage =
	 * new OverlayImage(basePath+lBandBottomImagePath, 100);
	 * getLogger().info("Image path "+basePath+graphicName); //create a transparent
	 * container for the bottom third of the screen. mainImage = new
	 * OverlayImage(0,0,srcWidth,srcHeight,100); //
	 * mainImage.addOverlayImage(secondImage,0,0);
	 * mainImage.addOverlayImage(holderImage,
	 * 0,srcHeight-holderImage.GetHeight(1.0)); //Add Text with a drop shadow //
	 * wowzaText = new OverlayImage("Haappy", 12, "SansSerif", Font.BOLD,
	 * Color.white, 66,15,100); // wowzaTextShadow = new OverlayImage("Haappy", 12,
	 * "SansSerif", Font.BOLD, Color.darkGray, 66,15,100); //
	 * mainImage.addOverlayImage(wowzaText, wowzaImage.GetxPos(1.0)+12, 54); //
	 * wowzaText.addOverlayImage(wowzaTextShadow, 1, 1);
	 * 
	 * //Fade the Logo and text independently //wowzaImage.addFadingStep(100,0,25);
	 * //wowzaImage.addFadingStep(0,100,25); //wowzaText.addFadingStep(0,100,25);
	 * //wowzaText.addFadingStep(100,0,25);
	 * 
	 * //do nothing for a bit mainImage.addFadingStep(50);
	 * wowzaImage.addImageStep(50); secondImage.addImageStep(50);
	 * holderImage.addImageStep(50); // wowzaText.addMovementStep(50);
	 * 
	 * //Fade the logo and text mainImage.addFadingStep(0,100,100);
	 * 
	 * 
	 * //Rotate the image while fading wowzaImage.addImageStep(1,37,25);
	 * wowzaImage.addImageStep(1,37,25); wowzaImage.addImageStep(1,37,25);
	 * wowzaImage.addImageStep(1,37,25); //Animate the text off screen to original
	 * location // wowzaText.addMovementStep(-75, 0, wowzaText.GetxPos(1.0), 54,
	 * 100);
	 * 
	 * //hold everything for a bit mainImage.addFadingStep(50);
	 * wowzaImage.addImageStep(50); secondImage.addImageStep(50);
	 * holderImage.addImageStep(50); // wowzaText.addMovementStep(50);
	 * 
	 * //Fade out mainImage.addFadingStep(100,0,50); wowzaImage.addImageStep(50);
	 * secondImage.addImageStep(50); wowzaImage.addFadingStep(50);
	 * holderImage.addFadingStep(50); secondImage.addFadingStep(50); //
	 * wowzaText.addMovementStep(50);
	 * 
	 * //Pinch back video videoBottomPadding.addAnimationStep(0,
	 * holderImage.GetHeight(1.0), 50); videoBottomPadding.addAnimationStep(200);
	 ...
	 * mainImage.addFadingStep(150); wowzaImage.addImageStep(150);
	 * secondImage.addImageStep(150); holderImage.addImageStep(150); //
	 * wowzaText.addMovementStep(50); videoBottomPadding.addAnimationStep(100);
	 * videoLeftPadding.addAnimationStep(100);
	 ***********************************************************************************************
	 * full screen overlay setup-
	 * getLogger().info("screen width="+srcWidth+" height = "+srcHeight);
	 * secondImage = new OverlayImage(basePath+secondGraphName,100);
	 * getLogger().info("getting bottom image"); holderImage = new
	 * OverlayImage(basePath+bottomImageName, 100);
	 * getLogger().info("bottom image created"); //create a transparent container
	 * for the bottom third of the screen. mainImage = new
	 * OverlayImage(0,0,srcWidth,srcHeight,100); //
	 * mainImage.addOverlayImage(secondImage,0,0);
	 * mainImage.addOverlayImage(holderImage, srcWidth-holderImage.GetWidth(1.0),0);
	 **************************************************************************************************
	 * 
	 * L band by separate images Create the Wowza logo image
	 * getLogger().info("screen width="+srcWidth+" height = "+srcHeight);
	 * secondImage = new OverlayImage(basePath+leftIamgeName,100);
	 * overlayScreenWidth = secondImage.GetWidth(1.0);
	 * getLogger().info("getting bottom image"); holderImage = new
	 * OverlayImage(basePath+bottomImageName, 100); overlayScreenHeight =
	 * holderImage.GetHeight(1.0); getLogger().info("bottom image created");
	 * //create a transparent container for the bottom third of the screen.
	 * mainImage = new OverlayImage(0,srcHeight,srcWidth,srcHeight,100);
	 * mainImage.addOverlayImage(secondImage,0,0);
	 * mainImage.addOverlayImage(holderImage,
	 * srcWidth-holderImage.GetWidth(1.0),srcHeight-overlayScreenHeight);
	 ********************************************************************************************************
	 * 
	 ...
	 * //create a transparent container for the bottom third of the screen.
	 * mainImage = new
	 * OverlayImage(0,srcHeight-overlayScreenHeight,srcWidth,overlayScreenHeight,100
	 * ); // mainImage.addOverlayImage(secondImage,0,0);
	 * mainImage.addOverlayImage(holderImage, srcWidth-holderImage.GetWidth(1.0),0);
	 *********************************************************************************************************
	 * 
	 * getLogger().info("screen width="+srcWidth+" height = "+srcHeight);
	 * secondImage = new OverlayImage(basePath+leftIamgeName,100); overlayWidth =
	 * secondImage.GetWidth(1.0); getLogger().info("getting bottom image");
	 * holderImage = new OverlayImage(basePath+bottomImageName, 100);
	 * overlayScreenHeight = holderImage.GetHeight(1.0);
	 * getLogger().info("bottom image created"); //create a transparent container
	 * for the bottom third of the screen. mainImage = new
	 * OverlayImage(0,srcHeight,srcWidth,srcHeight,100);
	 * mainImage.addOverlayImage(secondImage,0,0);
	 * mainImage.addOverlayImage(holderImage,
	 * srcWidth-holderImage.GetWidth(1.0),srcHeight-holderImage.GetHeight(1.0));
	 **************************************************************************************************************
	 * 
	 ...
	 * // mainImage.addOverlayImage(secondImage,0,0);
	 * 
	 * //Add Text with a drop shadow wowzaText = new OverlayImage(overlayText, 12,
	 * "SansSerif", Font.BOLD, Color.white, srcWidth,15,100); wowzaTextShadow = new
	 * OverlayImage(overlayText, 12, "SansSerif", Font.BOLD, Color.darkGray,
	 * srcWidth,15,100); //create a transparent container for the bottom third of
	 * the screen. overlayScreenHeight =
	 * holderImage.GetHeight(1.0)+wowzaText.GetHeight(1.0); mainImage = new
	 * OverlayImage(0,srcHeight-overlayScreenHeight,srcWidth,overlayScreenHeight,100
	 * ); mainImage.addOverlayImage(holderImage,
	 * srcWidth-holderImage.GetWidth(1.0),0); mainImage.addOverlayImage(wowzaText,
	 * wowzaImage.GetxPos(1.0)+12, overlayScreenHeight-wowzaText.GetHeight(1.0));
	 * wowzaText.addOverlayImage(wowzaTextShadow, 1, 1);
	 ****************************************************************************************************************
	 * 
	 * Bottom band setup secondImage = new OverlayImage(basePath + secondGraphName,
	 * 100); holderImage = new OverlayImage(basePath + bottomImageName, 100);
	 * wowzaText = new OverlayImage(overlayText, 12, "SansSerif", Font.BOLD,
	 * Color.white, srcWidth, 15, 100); wowzaTextShadow = new
	 * OverlayImage(overlayText, 12, "SansSerif", Font.BOLD, Color.darkGray,
	 * srcWidth, 15, 100); // create a transparent container for the bottom third of
	 * the // screen. overlayScreenHeight = holderImage.GetHeight(1.0) +
	 * wowzaText.GetHeight(1.0); mainImage = new OverlayImage(0, srcHeight -
	 * overlayScreenHeight, srcWidth, overlayScreenHeight, 100);
	 * mainImage.addOverlayImage(holderImage, srcWidth - holderImage.GetWidth(1.0),
	 * 0); mainImage.addOverlayImage(wowzaText, wowzaImage.GetxPos(1.0) + 12,
	 * overlayScreenHeight - wowzaText.GetHeight(1.0));
	 * wowzaText.addOverlayImage(wowzaTextShadow, 1, 1);
	 * 
	 * // do nothing for a bit mainImage.addFadingStep(50);
	 * wowzaImage.addImageStep(50); secondImage.addImageStep(50);
	 * holderImage.addImageStep(50); wowzaText.addImageStep(50);
	 * 
	 * // hold everything for a bit mainImage.addFadingStep(50);
	 * wowzaImage.addImageStep(50); secondImage.addImageStep(50);
	 * holderImage.addImageStep(50); wowzaText.addImageStep(50);
	 ...
	 * 
	 * // Pinch back video videoBottomPadding.addAnimationStep(0,
	 * overlayScreenHeight, 50); videoBottomPadding.addAnimationStep(200);
	 *************************************************************************************************************
	 * 
	 * tmider task for ads just for testing private void
	 * startAdsTimer(List<AdsModel> ads){ Timer timer = new Timer();
	 * timer.schedule(new TimerTask() { int adsCounter = 0;
	 * 
	 * @Override public void run() {
	 * setupadsImages(ads.get(adsCounter).getLogoFtpPath(),
	 * ads.get(adsCounter).getAdPlacement()); if (adsCounter < ads.size())
	 * adsCounter++; else adsCounter = 0; } }, 0, 30000); }
	 *
	 *
	 */
